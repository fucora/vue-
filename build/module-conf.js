...
var newModuleSrcArray = glob.sync(utils.resolve('src')+'/modules/**/*.html')
var moduleLi = []
for(var x in moduleSrcArray){
  moduleList.push(moduleSrcArray[x].split('/')[3])
}
for(var x in newModuleSrcArray){
  moduleLi.push(newModuleSrcArray[x])
}
